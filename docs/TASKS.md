...
| **paperview tex compile** | 🏗️ | Invokes the core Tectonic CLI adapter for a single `.tex` entry file, writes the generated PDF beside the source, and reports diagnostics; bundling/config policy and preview UI remain deferred |
