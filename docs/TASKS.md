...
- Bookmarks currently cover shared core persistence, TUI headless list/add/remove/prune commands, an interactive TUI picker that opens selected bookmarks near stored source lines, TUI reader shortcut `m` for creating bookmarks, GUI sidebar open/navigation to saved heading anchors, and GUI header bookmark creation; GUI source-line/scroll-progress fallback navigation is deferred.
