...
- In-Document Search currently covers shared case-insensitive line search, GUI header query with previous/next controls and rendered-text highlighting, and TUI `/`, `n`, and `N` navigation with highlighted match lines; selected-match emphasis in the GUI and exact rendered-line geometry are deferred.
