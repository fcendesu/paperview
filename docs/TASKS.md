...
- Bookmarks currently cover shared core persistence, TUI headless list/add/remove/prune commands, an interactive TUI picker that opens selected bookmarks near stored source lines, TUI reader shortcut `m` for creating bookmarks, a GUI sidebar section that opens bookmarked documents, and GUI header bookmark creation; precise GUI target navigation is deferred.
