...
| **paperview tex compile** | 🏗️ | Invokes the core Tectonic CLI adapter for a single `.tex` entry file, honors optional `tex_compiler_path`, writes the generated PDF beside the source, reports diagnostics, and supports `--open` for the generated PDF; bundling and embedded preview UI remain deferred |
