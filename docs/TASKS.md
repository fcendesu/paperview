...
- Bookmarks currently cover shared core persistence, TUI headless list/add/remove/prune commands, an interactive TUI picker that opens selected bookmarks near stored source lines, TUI reader shortcut `m` for creating bookmarks, and a GUI sidebar section that opens bookmarked documents; GUI bookmark creation and precise GUI target navigation are deferred.
