...
- Bookmarks currently cover shared core persistence, TUI headless list/add/remove/prune commands, an interactive TUI picker that opens selected bookmarks near stored source lines, and TUI reader shortcut `m` for creating bookmarks; GUI sidebar integration is deferred.
