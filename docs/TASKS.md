...
- LaTeX Support currently covers structured inline math spans, a dedicated display math block, GUI, TUI, HTML export, and text-first PDF export readable previews for common display math tokens, Greek letters, arrows, set/logic operators, sums/integrals, compact scripts, source-preserving display math, and inline math export metadata; full formula typesetting is deferred.
...
| **paperview export** | 🏗️ | HTML export writes standalone styled `.html` with static Mermaid and readable math previews; PDF export writes a text-first `.pdf` with basic layout, padded/wrapped tables, readable math preview text, and image metadata placeholders |
