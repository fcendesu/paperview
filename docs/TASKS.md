...
| **paperview tex compile** | 🏗️ | Invokes the core Tectonic CLI adapter for a single `.tex` entry file, honors optional `tex_compiler_path`, writes the generated PDF beside the source, and reports diagnostics; bundling and preview UI remain deferred |
