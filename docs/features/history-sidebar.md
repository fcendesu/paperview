...
- Prunes missing files from recent history when the GUI sidebar or TUI dashboard loads history.

Grouping by date and richer metadata are deferred.
...
- `History::prune_missing` removes entries whose paths no longer exist.
...
- GUI state loads persisted history on startup, prunes stale entries, records the initially loaded document, and saves the updated history.
...
- `paperview-tui` without a file opens the recent-files dashboard and prunes stale entries before rendering.

## Open Decisions

- Date grouping should be introduced once entries have durable timestamps.
- Explicit delete/clear history controls are deferred.
