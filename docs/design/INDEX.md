...
- **Search:** The GUI header includes a compact document search field with previous/next controls, match count, and highlighted rendered matches in the active reader. The TUI uses `/` for search entry, reports match position in the header, and highlights matched reader lines.
