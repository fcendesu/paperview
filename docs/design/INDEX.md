...
- **TUI Split Toggle:** The TUI uses `\` to toggle a first side-by-side reader
  ...
  pane through non-active tabs and `<` / `>` to resize the primary pane.
...
- **Split View:** The current GUI foundation divides the reader area vertically
  ...
  side pane and resizing for the primary pane.
    ...
    - **Resize:** The primary pane ratio is bounded from 30% to 70%, defaults
      to 50%, and can be changed by keyboard shortcuts or GUI divider drag.
      In the TUI, `<` / `>` changes the same bounded ratio.
...
clickable. In-document GUI links jump to matching headings, supported local
document links open as PaperView tabs, and external links use the platform
default opener. TUI links remain Markdown-shaped display text. Task-list items
render as clickable checkbox markers in the GUI when source writeback is
available and as Markdown-shaped `[x]` / `[ ]` markers in the TUI.
